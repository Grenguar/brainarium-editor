...
The selected directory is the vault. Main-process file operations resolve a real path and check containment. Path traversal is rejected. A symlink may be followed only when its target remains inside the vault; external/unavailable targets are shown as unavailable. Index Markdown (`.md`, `.markdown`), CSV, plain text (`.txt`), JSON, XML, HTML (`.html`, `.htm`), PDFs (`.pdf`), and image files (`.avif`, `.gif`, `.jpeg`, `.jpg`, `.png`, `.webp`). Text formats remain UTF-8 only; images are limited to 25 MB and must match an approved extension and content signature before their bytes reach the renderer. PDFs are read-only, limited to 64 MB, must start with the `%PDF-` signature, and reach the sandboxed renderer as verified bytes for the bundled local PDF.js renderer rather than as a filesystem URL. A native image import validates the selected bytes, atomically places them under `images/<active-note-parent-path>/`, removes the original only after that placement succeeds, reuses identical bytes, and inserts a vault-relative Markdown reference; name collisions never overwrite different data. Markdown and CSV have dedicated views; TXT, JSON, XML, and HTML are inert, exact-source, read-only previews; PDFs use a local paginated canvas view with page and zoom controls; images are inert, read-only fit/zoom previews. PDFs and images remain excluded from full-text search, MCP reads/writes, graphing, review, and copy-content actions. Copy always re-reads the active, vault-bound text document before placing its current text on the clipboard.

## Reading an opened vault on another device

An opt-in, read-only HTTP view of the already-opened vault exists so a tablet can read it over a private mesh network. It is off by default and starts only when the owner asks. It binds `127.0.0.1` and nothing else: it never binds `0.0.0.0` and never binds a mesh interface, so the process opens no externally reachable socket. Publishing that loopback port onto a tailnet is a deliberate step the owner takes outside Brainarium with `tailscale serve`; the app never configures it. Access additionally requires a pairing code generated per start, shown in the app, typed once on the reading device, held in an `HttpOnly; SameSite=Strict` cookie, and compared in constant time.

The served surface is reading only: no write, MCP, search, graph, or agent route is reachable by any URL. A URL resolves to a document by exact match against the scanned document list, so no path is concatenated and traversal cannot succeed; the boundary also re-asserts dot-directory rejection and imposes its own served-read size limit for text documents. Images and PDFs are served as verified bytes through the existing readers, so the surfaces that exclude them from search, MCP, graphing, review, and copy-content are unchanged — serving is a separate, explicitly authorized read. Vault HTML is served as escaped source and never as `text/html`, preserving the rule that raw HTML is inert. No response carries an absolute filesystem path, so the no-filesystem-paths rule holds outside the renderer too.

PDF export is the one operation that writes outside the vault, and it does so only to a destination the user picks in a native save dialog; vault containment checks therefore do not apply to that destination. The renderer supplies a relative path and nothing else — never markup, never a destination — and the main process re-reads the saved document, builds the printable HTML itself, and renders it in a hidden window with JavaScript disabled under a `default-src 'none'; img-src data:` policy. Images are inlined as `data:` URIs only after passing the same containment, size, extension, and signature checks as any other image read, subject to an aggregate inline budget; CSV exports are capped at 1,000 rows and 40 columns with the truncation stated in the document. Only the chosen file's basename returns to the renderer, so the no-filesystem-paths-in-the-renderer rule holds. Exports use the document as saved on disk, so the action is unavailable while a Markdown buffer is dirty. Image and PDF documents cannot be exported.
